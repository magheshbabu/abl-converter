...
public class ArrayConverter extends BaseConverter {
    ...
    private String processLine(String lineToProcess) {
       ...
       // String strToReplace = "";
        ...
        while (itr.hasNext()) {
            ...
            if (patternMatchFound(lineToProcess, tf)){
    ...
    private String handleNORMALReplacement(String lineToProcess, TableAndField tf) {
        ...
        strToFind = "(?i)(?<!-)(\\b" + tf.getTableName() + "\\s*\\.\\s*" + tf.getFieldName() + "\\b)\\s*(\\[)(.*?)(\\])";
   ...
   */
    ...
    private boolean patternMatchFound(String stringToScan, TableAndField tf) {
    	
    	String strToScan = stringToScan;
    	
    	Pattern p = Pattern.compile("(?i)(?<!-)(\\b" + tf.getTableName() + "\\s*\\.\\s*" + tf.getFieldName() + "\\b)\\s*(\\[)(.*?)(\\])");
    	Matcher m = p.matcher(strToScan);
